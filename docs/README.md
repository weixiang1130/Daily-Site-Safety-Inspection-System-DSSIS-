...
| 人員在場數、CCTV 違規辨識 | **待設備廠商串接**（見 `API_SPEC.md`） |
...
從同目錄的 `.otf` 轉出，**不可以直接改用 `.otf`**。原因與重現方式見該工具的說明：
Noto Sans TC 的 OTF 是 CID-keyed CFF，pdf-lib 依賴的 fontkit 對這種字型的子集化
會弄壞 FDSelect 對應，產出的 PDF 會變成「只有英數字、中文全空白」。
