...
|---|---|---|
| 熱指數（體感） | 27 / 32 / 41 / 54 °C | 職安署《高氣溫戶外作業勞工熱危害預防指引》，與美國 NWS 熱指數分級相同 |
| 噪音 | 80 / 85 / 90 dB | 職業安全衛生設施規則第 300 條 |
| PM2.5 | 15.5 / 35.5 / 54.5 μg/m³ | 環境部空氣品質指標 AQI（24 小時值） |
| PM10 | 51 / 101 / 255 μg/m³ | 環境部空氣品質指標 AQI（24 小時值） |

熱指數的單位確實是 °C，因為它是**體感溫度**——回答「感覺起來像幾度」。
以實測值驗證：溫度 31.4 °C、濕度 75.3% 代入 NWS 迴歸式得 104.9 °F ＝ 40.5 °C，
與廠商回報的 42.8 相近；若該數字是華氏，對應的會是 105 左右而非 42.8。
上面的門檻 27／32／41／54 也正是 NWS 的 80／90／105／130 °F 換算而來。

但它和「溫度」並排、單位又同為 °C，很容易被誤讀成氣溫，因此標籤明確寫成
「熱指數 體感」，並在滑鼠停留時說明。
...
實測（2026-08-20）：同一時刻由公司網路取像正常（401 挑戰 → 200，382 KB），
由 Netlify（美東）卻在**還沒帶帳密的第一次請求**就被回 403。也就是監視器
主機或其前方防火牆有來源 IP／地區限制，而 Netlify 函式沒有固定的對外 IP
可以加白名單。
...
畫面超過 `SNAPSHOT_MAX_AGE_SEC`（180 秒）算過期。直連也失敗時仍會回傳舊
畫面，但會附上 `X-Snapshot-Age`，牆上直接顯示「N 分鐘前」——停止推送後
必須看得出來，否則會一直顯示同一張而沒人察覺。

推送程式用**專屬權杖**而非設備廠商的權杖：它是我們自己的內網代理，
權限範圍也只有上傳畫面這一件事。

搬回內網之後這個限制自然消失，屆時可以改回直連，程式碼兩條路徑都留著。

三個實作上必須注意的地方：

1. **前端不用 `<img src>` 直接載入。** 那樣失敗只會留下一片黑，看不出是
   連線問題、權限問題還是鏡頭壞了。改為自己取回內容，失敗就把原因寫在框裡。
2. **函式有 8.5 秒總預算。** Netlify 函式只有 10 秒，而一次取像要兩趟
   （換 nonce、帶憑證），遇到 401 還要再一趟。預算用完就提早回報逾時，
   不要讓平台把函式砍掉——那樣前端只會拿到一個沒有訊息的錯誤。
3. **換圖間隔 15 秒。** 一張快照約 385 KB 且全部經由後端代理，牆上的畫面
   整天開著，換得太勤會吃掉可觀的流量與函式呼叫次數。

### 戰情室的工地範圍

工地選單預設「全部工地」：缺失統計、災害類別分布、廠商缺失排行與趨勢
都看全公司。但**環境數據、進出場人次與監視畫面固定以主場站為準**
（`PRIMARY_SITE_CODE`）——把多個工地的即時數值混在一起沒有意義，
而監視器本來就只接一個工地。其餘工地的環境以一行帶過。
