...
|---|---|---|
| 熱指數（體感） | 26.7 / 32.2 / 40.6 / 54.4 °C | 職安署《高氣溫作業熱危害預防指引》附表二 |
| 噪音 | 80 / 85 / 90 dB | 職業安全衛生設施規則第 300 條 |
| PM2.5 | 15.5 / 35.5 / 54.5 μg/m³ | 環境部空氣品質指標 AQI（24 小時值） |
| PM10 | 51 / 101 / 255 μg/m³ | 環境部空氣品質指標 AQI（24 小時值） |

> **門檻不可四捨五入。** 附表二明定第三級為「40.6 以上，未達 54.4」，
> 寫成 41 會讓熱指數 40.6～41 的情況少判一級。四個分界正是 NWS 的
> 80／90／105／130 °F 換算值，指引本身的驗算例也吻合：溫度 32.2 °C、
> 濕度 70% 對應熱指數 40.6，即第三級。
...
達第一級即在儀表板最上方顯示警示帶，內容取自指引附表二與附表三
（`backend/cloud/lib/heat-guidance.ts`）：

- **本級重點**：這一級相較前一級「多」要求的事。八項每級都要做的基本措施
  不列在牆上——全列出來反而看不出這一級的重點，完整清單留給彈出提醒。
- **操作要點**：指引裡有明確數字的幾件事（補水頻率、重體力作業休息時間）。
- **注意**：附表二備註三規定陽光直照或穿不透氣防護衣應提升一級，
  這裡直接算出會提升到第幾級。工地戶外作業多屬前者，只顯示現值會讓現場低估；
  但是否成立要由現場認定，因此並列而不直接取代判定結果。

等級**上升**時另外彈出提醒，列出該級全部應辦措施。只在上升時彈出——維持在
同一級還一直跳，很快就沒有人會看。牆上沒有人會去關掉它，因此 40 秒後自動收起。

未達第一級時警示帶完全不顯示。常態顯示的警示會被當成背景而失去作用。
...
1. **整頁不出現捲軸。** 牆上的畫面沒有人會去滑，看不到就等於沒有。
   ...
   分頁是平均分配而非填滿——22 筆、容量 21 會變成第二頁只剩一列，
   牆上輪到那一頁就只有孤零零一行。
   排行的橫條則是「先畫再修」：畫完量實際高度，塞不下的移掉。事前估算
   不可靠，第一次渲染時還沒有列可以量。

   **但一屏只保證給牆上的大螢幕。** 低於 800 CSS 高度（例如 1366×768 的
   筆電）會還原正常捲動——硬壓成一屏的結果是每一塊都被擠到讀不出來，
   那比捲動更糟。介於中間的高度（如 1080 螢幕開 125% 縮放的 864）則收掉
   趨勢圖並縮小數字字級：趨勢是回顧性資訊，少了它不影響當下判斷，
   排行被壓扁卻是直接失效。
...
- **快照的儲存鍵帶工地代碼**（`CCTV_SITE_CODE`，未設定時沿用
  `PRIMARY_SITE_CODE`）。頻道 1 幾乎每台 NVR 都有，只用頻道編號當鍵的話，
  第二個工地的推送程式會直接蓋掉第一個工地的畫面，而牆上仍掛著原本的工地
  名稱、時間也顯示「幾秒前」——因為那張圖確實是新的，完全看不出錯。
  推送端以 `SITE_CODE` 宣告自己是哪個工地。
- **人數解析不到主場站時不顯示，不退回全公司加總。** 代碼打錯、改名或工地
  停用時退回加總，等於把全公司總和放到單一工地名稱底下，疏散時會照著一個
  過大的數字點名——那正是這段程式要避免的事。
...
- **地端沒有遷移工具**，`create_all()` 只建表不加欄位。啟動時會比對模型與
  實際資料表，自動補上缺少的欄位——清單由模型推導，不要另外手工維護一份：
  手工清單漏掉的欄位不會有任何人發現（`sites.department` 就這樣在地端缺了
  很久，而共用前端的工地下拉正是依它分組的）。
- **狀態標籤一律用 `statusTag()`，必填姓名一律用 `requireName()`**（都在
  `frontend/common.js`）。各頁自己寫的話很快就會分岔：先前兩份狀態判斷都
  漏掉 `verified`，把「已複驗」顯示成紅色的待處理。
